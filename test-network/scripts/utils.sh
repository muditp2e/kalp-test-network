...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"kalptantra\")"
    ...
    println "   \033[0;32mup\033[0p2eprom -ca -r -d -s -verbose"
    println "   \033[0;32mup createChannel\033[0m -ca -c -r -d -s -verbose"
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c kalptantra -s couchdb "
